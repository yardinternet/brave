...
@if ($primaryNavigation->isNotEmpty())
	<nav class="nav-primary hidden items-center lg:flex" aria-label="{{ __('Primaire navigatie', 'sage') }}">
		<ul class="nav flex gap-x-4 xl:gap-x-8 h-full list-reset items-center justify-center">
			@foreach ($primaryNavigation->all() as $item)
				<li @class([
					...
					@if ($item->children)
						<ul
							class="sub-menu list-reset ease-base invisible absolute min-w-48 -translate-y-3 bg-white opacity-0 shadow-md transition-all group-has-aria-expanded:visible group-has-aria-expanded:translate-y-0 group-has-aria-expanded:opacity-100">
